...
| **Ревизия** | 1.4 (черновик для согласования) |
...
| TC-56 | (предлагается, L3, до выпуска 0.9.3) H5 `STM32H563ZI`: CMSIS/HAL; FreeRTOS `external` из FreeRTOS-Kernel с `ARM_CM33_NTZ`; `crc_enable: true` с шаблоном | Цели CMSIS/HAL H5; `FreeRTOS::ARM_CM33_NTZ` (для NTZ-порта stm32-cmake не добавляет суффикс `::NON_SECURE`); CRC по п. 4.15.8 `(р.1.2)` |
...
| 4.15.1, 4.15.3, 4.15.5 | PB; CRC | TC-28, TC-36, TC-43 |
...
| 10.2.9 | Отсутствие обёртки ядра Arduino — предупреждение или ошибка? | Открыт | Изучить до выпуска 0.9.3. Сценарии CI используют синтетическую обёртку `tests/fixtures/project/Arduino/Core/CMakeLists.txt`; сценария без обёртки ядра нет, поведение сборки без неё не проверено Направление изучения (решение заказчика): конфигурация проекта, в которой `CMakeLists.txt` подключаются напрямую из `Arduino_Core_STM32/cores` и из `Arduino_Core_STM32/libraries` по отдельности, без обёрток проекта `(р.1.4)` | 4.9.4, 4.9.5, TC-30 |
