...
| **Ревизия** | 1.8 (черновик для согласования) |
| **Дата формирования** | 27.09.2026 |
...
| **Целевая версия** | 0.9.3 (выпуск с небольшими исправлениями; проверка в CI GitHub Actions; GitLab CI — после 0.9.3, вопрос 10.2.19) `(р.1.4)` |
| **Целевая платформа** | CMake ≥ 3.19 (проверяются 3.19.8 и 3.28.3), Arm GCC (проверяются xPack 13.3.1-1.1, 14.2.1-1.1, 15.2.1-1.1), Ninja/Make, Mike Farah yq v4 (проверяется 4.44.3), Python 3 (для CRC), backend stm32-cmake (ObKo) или Arduino Core STM32 (проверяется 2.12.0); хост — Linux и Windows; CI — GitHub Actions `(р.1.4)` |
| **Связанные документы** | README.md; анализ использования stm32-cmake (ревизия `ecc5acc`, закреплена в `ci/dependencies.lock.json`) и его перепроверка по исходникам; `docs/ru/reference/0.9.2/*` (справочник опций `CFG-*`); `docs/ru/reference/0.9.2/semantics.md`; `docs/ru/errata/*`; `docs/ru/testing.md`; `docs/ru/firmware-testing.md`; `docs/ru/firmware-plan.md`; CHANGELOG.md; TODO.md |
...
| 1.2 | 26.09.2026 | Учтены результаты анализа использования stm32-cmake, перепроверенного по исходникам закреплённой ревизии `ecc5acc`, и решения заказчика о включении доработок в 0.9.3. Добавлены: исправление external FreeRTOS (E007) с проверкой на дереве Cube и FreeRTOS-Kernel; выбор и проверка `mcu_core` для H7; таблица портов FreeRTOS для новых семейств; ранняя проверка компонентов HAL/FreeRTOS; уточнённая проверка RAM; предупреждение о неприменённых heap/stack; проверка секции CRC на Configure; артефакт `srec`. Описана предвыпускная проверка H7 и H5. Закрыт вопрос 10.2.8, добавлен 10.2.18. |
| 1.3 | 26.09.2026 | Закрыт вопрос 10.2.18: в lock-файл и образ CI добавлены STM32CubeH7 1.13.0, STM32CubeH5 1.7.0 и FreeRTOS-Kernel 11.3.1, MCU сценариев подтверждены базой stm32-cmake `ecc5acc`. В приложение E внесены результаты пробного Configure версии 0.9.2 на H7 и H5. Добавлен вопрос 10.2.19 о GitLab CI в приёмке 0.9.3. |
| 1.4 | 26.09.2026 | Закрыт вопрос 10.2.19: версия 0.9.3 принимается по GitHub Actions, конвейер GitLab CI и TC-53 переносятся на следующую версию. Для вопросов 10.2.9 и 10.2.17 записаны направления изучения: подключение `CMakeLists.txt` из `Arduino_Core_STM32/cores` и `libraries` и сборка прошивки с CRC для `STM32H503C8`. |
| 1.5 | 26.09.2026 | Закрыты вопросы 10.2.9 и 10.2.17. Промежуточный образ CRC формируется из секций ELF с адресом загрузки в регионе `FLASH` (п. 4.15.9); ограничение H5 снято; добавлены TC-63, TC-64 и повторение локальных проверок перед выпуском (п. 8.8.7). Прямое подключение `CMakeLists.txt` Arduino Core и переход на CMake ≥ 3.21 перенесены в 0.10.x; результаты прототипа записаны в п. 10.3.5. |
| 1.6 | 26.09.2026 | Записаны результаты доработок 0.9.3 (ветка `claude/release-0.9.3`, коммиты `4cd3404`…`97682e3`) и решения заказчика. Закрыт вопрос 10.2.16: при `profiles_file` доступны только внешние профили, о встроенных выводится предупреждение. Имена `lss`/`map` остаются как в 0.9.2; BIN строится из секций FLASH, как образ CRC; добавлено предупреждение о символе `_sstack` (п. 4.11.10). Прошивки H7 и H5 запускаются в Renode на минимальных моделях, QEMU для них не применим. Реализованные тест-кейсы переведены из «(предлагается)», добавлены TC-65, TC-66; приложения E и F обновлены. |
| 1.7 | 27.09.2026 | Завершающие доработки 0.9.3 по решению заказчика: баннер версии «stm32-cmake-yml версия: …» (п. 4.2.2), автоматическая проверка TC-52 в сборщике прошивок, правило очистки кэша для 0.9.2 в документации (п. 3.6.6), исправлен комментарий о `STM32::Semihosting` и описаны FloatPrint/FloatScan (п. 4.10.2, 4.10.4). Предложения для 0.10.x записаны в TODO.md, в ТЗ не вносятся. |
| 1.8 | 27.09.2026 | По решению заказчика матрица прошивок расширяется на семейства F0, F4, G4, F7 с полным набором профилей; цели добавляются по одной с полной локальной проверкой (п. 8.8.8, TC-67). Добавлена цель F0 `STM32F030R8T6`. В режиме `external` к цели порта добавляется `portasm.c`, если stm32-cmake его не подключил (п. 4.8.6; найдено на `ARM_CM0` FreeRTOS-Kernel 11). |



### Изменения ревизии 1.8

Изменённые и новые пункты ревизии 1.8 помечены `(р.1.8)`.

| Пункты | Тип | Суть |
| :--- | :---: | :--- |
| 4.8.6 | изм. | `portasm.c` порта при внешнем FreeRTOS |
| 8.8.8 | нов. | Матрица прошивок по семействам F0, F4, G4, F7 |
| 8.7 | нов. | TC-67 |
| Раздел 9, приложения E, F | изм. | Строки 4.8.6, 8.8.8; модели Renode; объём проверок |

### Изменения ревизии 1.7

Изменённые и новые пункты ревизии 1.7 помечены `(р.1.7)`.

| Пункты | Тип | Суть |
| :--- | :---: | :--- |
| 4.2.2 | изм. | Текст баннера версии |
| 8.7 | изм. | TC-52 автоматизирован |
| Приложение E | изм. | Строки 3.6.6, 4.10.2, 4.10.4, 4.15.7 |

### Изменения ревизии 1.6

Изменённые и новые пункты ревизии 1.6 помечены `(р.1.6)`.

| Пункты | Тип | Суть |
| :--- | :---: | :--- |
| 3.4.11 | изм. | Только внешние профили; предупреждение о встроенных (вопрос 10.2.16) |
| 4.8.8 | изм. | Совместимость обёртки с внешним FreeRTOS подтверждена сборкой; отсутствие обёртки — ошибка |
| 4.11.10 | нов. | Предупреждение об отсутствующем `_sstack` |
| 4.14.2 | изм. | BIN из секций FLASH; имена `lss`/`map` как в 0.9.2 |
| 8.8.5 | изм. | H7 и H5 — сборка и запуск в Renode на минимальных моделях |
| 8.7 | изм./нов. | TC-22, TC-43, TC-46 … TC-51, TC-54 … TC-64 реализованы; TC-65, TC-66 |
| 10.2.16 | При одновременном наличии встроенной секции `profiles:` и `profiles_file` какие профили доступны: только внешние (как в 0.9.2), объединение с заменой одноимённых профилей внешними или ошибка? | Закрыт | Только внешние, как в 0.9.2; о встроенных профилях выводится предупреждение со списком `(р.1.6)` | 3.4.11, TC-48 |
| Раздел 9, приложения E, F | изм. | Строки 3.4.11, 4.11.10, 4.14.2, 8.8.5; состояние доработок 0.9.3; объём проверок |

### Изменения ревизии 1.5

Изменённые и новые пункты ревизии 1.5 помечены `(р.1.5)`.

| Пункты | Тип | Суть |
| :--- | :---: | :--- |
| 4.9.4 | изм. | Поведение 0.9.3 без изменений; прямое подключение Core — 0.10.x |
| 4.15.1, 4.15.8, 5.6.2 | изм. | Образ CRC по секциям Flash; ограничение H5 снято; интерфейс скрипта с ELF |
| 4.15.9 | нов. | Состав промежуточного образа CRC |
| 8.8.7 | нов. | Проверка образа CRC и повтор локальных проверок перед выпуском |
| 8.7 | нов. | TC-63, TC-64 |
| 10.2.9, 10.2.17 | изм. | Вопросы закрыты |
| 10.3.5 | нов. | Результаты прототипа прямого подключения Arduino Core |
| Раздел 9, приложения B, E | изм. | Строки CRC и Arduino |

### Изменения ревизии 1.4

Изменённые и новые пункты ревизии 1.4 помечены `(р.1.4)`.

| Пункты | Тип | Суть |
| :--- | :---: | :--- |
| Реквизиты | изм. | Приёмка 0.9.3 — только GitHub Actions |
| 8.5.6, TC-53 | изм. | Эквивалентность с GitLab CI — после 0.9.3 |
| 10.2.9, 10.2.17 | изм. | Направления изучения |
| 10.2.19 | изм. | Вопрос закрыт |
| Раздел 9, приложение E | изм. | Строки 8.5.6 |

### Изменения ревизии 1.3

Изменённые и новые пункты ревизии 1.3 помечены `(р.1.3)`.

| Пункты | Тип | Суть |
| :--- | :---: | :--- |
| 8.8.1 | изм. | Закреплённые версии STM32CubeH7, STM32CubeH5 и FreeRTOS-Kernel |
| 10.2.18 | изм. | Вопрос закрыт: версии и MCU сценариев |
| 10.2.19 | нов. | Вопрос: входит ли GitLab CI в приёмку 0.9.3 |
| Приложения E, F | изм. | Состояние доработок по пробному Configure; версии окружения |

### Изменения ревизии 1.2

Изменённые и новые пункты ревизии 1.2 помечены `(р.1.2)`.

| Пункты | Тип | Суть |
| :--- | :---: | :--- |
| 3.7.3 | изм. | `freertos_version`: неизвестное значение — как `external`; артефакт `srec` |
| 4.4.4 | изм. | Таблица портов FreeRTOS по семейству и ядру; незнакомое семейство — предупреждение |
| 4.7.3, 4.8.3 | изм. | Цели CMSIS и CMSIS-RTOS с учётом ядра |
| 4.7.8, 4.7.9 | нов. | Выбор `mcu_core` по базе stm32-cmake; ранняя проверка компонентов HAL |
| 4.8.6 | изм. | External FreeRTOS: поиск без компонента семейства (исправление E007) |
| 4.8.7, 4.8.8 | нов. | Ранняя проверка компонентов FreeRTOS; external FreeRTOS с CMSIS-RTOS |
| 4.10.4 | нов. | Float printf/scanf newlib-nano через `link_libraries` |
| 4.11.9 | нов. | Предупреждение: размеры heap/stack не применяются к скрипту |
| 4.13.4, 4.14.2, 4.15.2, 4.15.4 | изм. | Проверка RAM с CCRAM/RAM_SHARE и ядром, «не проверялось»; `srec`, `OUTPUT_NAME`; проверка секции CRC на Configure; Flash с ядром |
| 4.16.1, 4.16.2, 5.2.1 | изм. | Новые ошибки и предупреждения; используемые функции и цели stm32-cmake |
| 8.2.1 | изм. | FreeRTOS-Kernel, STM32CubeH5 и STM32CubeH7 в lock-файле |
| 8.8 | нов. | Предвыпускная стабилизация 0.9.3: H7, H5, external FreeRTOS |
| 8.7 | изм./нов. | TC-22, TC-25, TC-27, TC-28, TC-41 изменены; TC-54 … TC-62 |
| 9, 10 | изм. | Матрица; вопрос 10.2.8 закрыт; добавлен 10.2.18 |
| Приложения A, E, F | изм. | Значения по умолчанию; доработки 0.9.3; тестовое окружение |
...
3.4.11. При заданном `profiles_file` ДОЛЖНЫ быть доступны только профили внешнего файла, как в 0.9.2: встроенный профиль, в том числе одноимённый, не учитывается. Если в основном файле есть непустая секция `profiles:`, ДОЛЖНО выводиться предупреждение со списком её профилей и рекомендацией перенести их во внешний файл или удалить секцию (вопрос 10.2.16). В 0.9.2 предупреждения нет. `[U]` `(р.1.6)`
...
| `build_artifacts` (элемент) | `bin`, `hex`, `srec`, `map`, `lss` `(р.1.2)` | элемент пропускается |
| `crc_algorithm` | `STM32_HW_DEFAULT` | `STM32_HW_DEFAULT` |
| `freertos_version` | `cube`, `external` | как `external` (совместимость с 0.9.2) `(р.1.2)` |
...
4.2.2. После чтения конфигурации ДОЛЖЕН выводиться баннер с версией фреймворка в виде `stm32-cmake-yml версия: <версия>`; результат сравнения п. 4.2.3 выводится строкой `Версия в конфигурации: <версия> (<итог>)`. В 0.9.2 строки имели вид `Framework : …` и `Config : …`. `[U]` `(р.1.7)`
...
4.4.4. Если IOC включает FreeRTOS и `use_freertos` не задан, FreeRTOS ДОЛЖЕН включаться; явное `false` в YAML, профиле или override подавляет включение. При включении по умолчанию `freertos_version: cube`, `cmsis_rtos_api` — из IOC, `freertos_components` — порт по семейству и ядру MCU и `Heap::4` (таблица в приложении A). Для семейства, отсутствующего в таблице, ДОЛЖНО выводиться предупреждение с фактически выбранным портом `ARM_CM4F`. В 0.9.2 таблица не содержит C0, U0, H5, L5, U5, WL и ядер M4/M0PLUS, а незнакомое семейство молча получает `ARM_CM4F`. `[U]` `(р.1.2)`
...
4.7.3. При `use_cmsis: true` ДОЛЖЕН подключаться пакет CMSIS семейства с ядром по п. 4.7.8, а цель CMSIS устройства `CMSIS::STM32::<устройство>[::<ядро>]` ДОЛЖНА связываться с исполняемой целью (п. 4.11). `[U]` `(р.1.2)`
...
4.7.8. Ядро MCU (`mcu_core`) ДОЛЖНО определяться по списку ядер stm32-cmake (`stm32_get_cores`): (а) пустой список — ядро не используется; (б) один элемент и `mcu_core` не задан — используется этот элемент (все H7, включая одноядерные, — `M7`; WB и MP1 — `M4`) с сообщением в журнале; (в) несколько элементов и `mcu_core` не задан — ошибка Configure со списком ядер; (г) `mcu_core`, отсутствующий в списке (в том числе для семейства без ядер), — ошибка Configure со списком допустимых значений. Выбранное ядро ДОЛЖНО применяться ко всем целям CMSIS, HAL, FreeRTOS, CMSIS-RTOS, к перехвату startup/system (п. 4.6.6) и к запросам памяти (п. 4.13.4, 4.15.4). В 0.9.2 ядро по умолчанию не выбирается: проект H7 без `mcu_core` ссылается на несуществующую цель CMSIS. `[U]` `(р.1.2)`

4.7.9. После подключения HAL каждый элемент `hal_components` ДОЛЖЕН проверяться на существование цели `HAL::STM32::<семейство>[::<ядро>]::<компонент>`; отсутствующий компонент — ошибка Configure с именем компонента и семейства. В 0.9.2 ошибка возникает только на Generate с диагностикой CMake. `[U]` `(р.1.2)`
...
4.8.3. `cmsis_rtos_api: v1` и `v2` ДОЛЖНЫ подключать обёртки `CMSIS::STM32::<семейство>[::<ядро>]::RTOS` и `::RTOS_V2`; `none` обёртку не подключает; неизвестное значение — по п. 3.7. В 0.9.2 имя обёртки строится без ядра и для H7 не существует. `[U]` `(р.1.2)`
...
4.8.6. Режим `freertos_version: external` предназначен для подключения FreeRTOS не из STM32Cube: старых проектов с собственной копией FreeRTOS и семейств, пакеты Cube которых FreeRTOS не содержат (H5, U5). Фреймворк ДОЛЖЕН вызывать `find_package(FreeRTOS COMPONENTS <порт>)` без компонента семейства, чтобы stm32-cmake искал FreeRTOS в `FREERTOS_PATH` (переменная CMake или окружения) и создавал цели `FreeRTOS::<порт>` и `FreeRTOS::<компонент>`. Поддерживаются раскладка дерева Cube (`Middlewares/Third_Party/FreeRTOS`, `Source/...`) и раскладка FreeRTOS-Kernel (`include/`, `portable/GCC/<порт>[/non_secure]`). Отсутствие `FREERTOS_PATH` или ненайденные файлы порта — ошибка Configure. В 0.9.2 фреймворк передаёт компонент семейства, stm32-cmake переключается на поиск в Cube и создаёт цели `FreeRTOS::STM32::<семейство>::*`, а фреймворк ссылается на `FreeRTOS::<порт>` — Generate завершается ошибкой (E007). `[U]` `(р.1.2)` Если в каталоге порта есть `portasm.c`, а stm32-cmake не включил его в исходники цели `FreeRTOS::<порт>` (так для `ARM_CM0` FreeRTOS-Kernel 11, где в нём `ulSetInterruptMask`), фреймворк ДОЛЖЕН добавить этот файл к цели порта. `(р.1.8)`

4.8.7. После подключения FreeRTOS каждый элемент `freertos_components` ДОЛЖЕН проверяться на существование соответствующей цели; отсутствующий компонент — ошибка Configure с именем компонента и пространством целей (`FreeRTOS::STM32::<семейство>[::<ядро>]` или `FreeRTOS`). `[U]` `(р.1.2)`

4.8.8. Обёртки CMSIS-RTOS (п. 4.8.3) ДОЛЖНЫ работать и с внешним FreeRTOS: исходники обёртки берутся из пакета CMSIS семейства, заголовки FreeRTOS — из целей внешнего FreeRTOS. Совместимость подтверждена до выпуска 0.9.3 (п. 8.8.4): Configure с FreeRTOS-Kernel 11.3.1 и `v2` (TC-58) и пробная сборка прошивки `freertosQueue` с CubeF1 `CMSIS_RTOS_V2` при настройках FreeRTOSConfig.h, которых требует обёртка; выполнение такой прошивки не проверялось. Если цели обёртки нет (пакет Cube без FreeRTOS, например H5 и U5), Configure ДОЛЖЕН завершаться ошибкой с рекомендацией `cmsis_rtos_api: none`. `[N]` `(р.1.6)`
...
4.9.4. Обёртка ядра ДОЛЖНА подключаться из `arduino.core_cmake_dir` (по умолчанию `Arduino/Core`) через `add_subdirectory`; отсутствие `CMakeLists.txt` — предупреждение. В 0.9.3 поведение 0.9.2 сохраняется; подключение `CMakeLists.txt` из `cores` и `libraries` Arduino Core напрямую — в 0.10.x (вопрос 10.2.9, п. 10.3.5). `arduino.use_core_main`, если задан, экспортируется в кэш `USE_CORE_MAIN`. `[R]` `(р.1.5)`
...
4.10.4. Поддержка чисел с плавающей точкой в `printf`/`scanf` newlib-nano ДОЛЖНА подключаться через `link_libraries: [STM32::Nano::FloatPrint]` и `STM32::Nano::FloatScan` (цели stm32-cmake) без отдельного ключа YAML; способ описывается в документации к `use_newlib_nano`. `[R]` `(р.1.2)`
...
4.11.9. Если `heap_size` или `stack_size` получены из YAML, профиля, override или IOC, а итоговый скрипт компоновщика не генерируется из шаблона `.ld.in` (скрипт stm32-cmake или явный `linker_script`), ДОЛЖНО выводиться предупреждение о том, что значения к скрипту не применяются. Для скрипта stm32-cmake в предупреждении указываются его собственные размеры (`stm32_get_memory_info`: 0x200/0x400 байт, для MCU с 2K RAM — 0x100/0x200). Значения ручного режима по умолчанию (п. 4.4.7) предупреждения не вызывают. `[N]` `(р.1.2)`

4.11.10. Если файл startup, который попадёт в сборку (из пакета CMSIS или перехваченный из `sources`, п. 4.6.6), использует символ `_sstack` (граница стека ARMv8-M, `msr MSPLIM`, например startup STM32CubeH5 1.7.0), а итоговый скрипт компоновщика его не определяет (скрипт stm32-cmake или шаблон/явный скрипт без `_sstack`), ДОЛЖНО выводиться предупреждение Configure с именем startup-файла и строкой `_sstack = _estack - _Min_Stack_Size;` для шаблона. Причина: иначе компоновка завершается малопонятной ошибкой `undefined reference to _sstack`. `[N]` `(р.1.6)`
...
4.13.4. `validate_linker_script: true` (по умолчанию) при backend stm32-cmake ДОЛЖЕН сравнивать сумму распознанных RAM-областей (`rw`, не с нулевого адреса) итогового скрипта с суммой RAM, CCRAM и RAM_SHARE MCU по `stm32_get_memory_info` с учётом ядра (п. 4.7.8) и выводить соотношение. Несовпадение — не ошибка; отсутствие распознанных областей — предупреждение. Если скрипт формирует stm32-cmake (п. 4.11.5), проверка не выполняется и в журнал ДОЛЖНО выводиться «не проверялось». При backend arduino проверка пропускается с сообщением. В 0.9.2 для скрипта stm32-cmake выводится равенство без проверки, а CCMRAM (F3, G4) даёт ложное превышение. `[U]` `(р.1.2)`

### 4.14. Артефакты после сборки

4.14.1. Для каждой цели ДОЛЖНА добавляться команда вывода размера образа (функция toolchain). `[R]`

4.14.2. Элементы `build_artifacts`: `hex` и `srec` ДОЛЖНЫ добавлять преобразование ELF функциями toolchain (`stm32_generate_hex_file`, `stm32_generate_srec_file`); `bin` — образ из секций ELF с адресом загрузки в регионе FLASH по правилам п. 4.15.9 (без исключения секции CRC; при `crc_enable` — после внедрения CRC), чтобы секция вне Flash не раздувала файл; регион FLASH берётся из шаблона или явного скрипта, для скрипта stm32-cmake — из `stm32_get_memory_info` с учётом ядра; если регион или Python недоступны — `stm32_generate_binary_file` с предупреждением; `lss` — листинг `objdump -h -S`; `map` — по п. 4.5.7. Имена `bin`, `hex`, `srec` строятся как у функций stm32-cmake (`OUTPUT_NAME` цели на момент вызова, иначе имя цели); имена `lss` и `map` — по имени цели, как в 0.9.2 (решение заказчика; выбор имён по итоговому имени ELF отложен с учётом нескольких backend). Неизвестный элемент — по п. 3.7 (в 0.9.2 пропускается без предупреждения); пустой список не отключает основную цель. В 0.9.2 `bin` создаётся `objcopy -O binary` и для ELF с секцией вне Flash занимает сотни мегабайт. `[U]` `(р.1.6)`
...
4.15.1. `crc_enable: true` (значения `true`, `on`, `1`, `yes` без учёта регистра) ДОЛЖЕН добавлять post-build последовательность: (а) формирование промежуточного образа Flash по п. 4.15.9; (б) расчёт CRC скриптом `scripts/stm32_crc.py`; (в) запись результата в секцию `crc_section_name` (по умолчанию `.checksum`) ELF через `objcopy --update-section`. В 0.9.2 образ формируется командой `objcopy -O binary --gap-fill 0xFF` без секции CRC и служебных секций `.ARM.attributes`, `.comment`, `.debug_*`. `[U]` `(р.1.5)`

4.15.2. Секцию CRC размером 4 байта ДОЛЖЕН предоставлять скрипт компоновщика; фреймворк её не создаёт. При `crc_enable: true` фреймворк ДОЛЖЕН при Configure: (а) для скрипта stm32-cmake, в котором секции заведомо нет, завершаться ошибкой с рекомендацией шаблона `.ld.in` или явного скрипта; (б) для шаблона и явного скрипта — искать имя `crc_section_name` в тексте итогового скрипта и при отсутствии выводить предупреждение. В 0.9.2 выводится только напоминание, а отсутствие секции проявляется на post-build. `[U]` `(р.1.2)`
...
4.15.4. Предельный размер промежуточного образа ДОЛЖЕН определяться `flash_size` (по умолчанию `auto`): при `auto` — объём Flash MCU по `stm32_get_memory_info` с учётом ядра (п. 4.7.8), при backend arduino — строка `FLASH … LENGTH` скрипта компоновщика. Если размер определить нельзя (Arduino), CRC отключается с предупреждением. `[U]` `(р.1.2)`
...
4.15.8. Сборка с `crc_enable: true` ДОЛЖНА выполняться для всех семейств, включая STM32H5, без особых ограничений: раздувание промежуточного образа, которое в 0.9.2 проявлялось на H5 из-за секций вне Flash, устраняется п. 4.15.9. Проверено на `STM32H503CB`: в 0.9.2 инициализированная секция в резервной SRAM (`0x40036400`) дала промежуточный образ 896 МиБ и заглушку `0x00000000` (E006); образ по п. 4.15.9 содержит только секции Flash, CRC совпадает с независимым расчётом. `[U]` `(р.1.5)`

4.15.9. Промежуточный образ ДОЛЖЕН строиться из секций ELF, размещаемых в памяти и имеющих содержимое (все, кроме `NOBITS`), кроме секции `crc_section_name`, у которых адрес загрузки (LMA) лежит в регионе `FLASH` итогового скрипта компоновщика (`ORIGIN`, `LENGTH`; п. 4.11.7). Образ начинается с наименьшего адреса загрузки этих секций и заканчивается концом последней; промежутки заполняются `0xFF`. Секции с адресом загрузки вне региона (например, инициализированные данные в резервной SRAM) в образ не входят и перечисляются в журнале сборки. Для ELF без таких секций образ ДОЛЖЕН побайтно совпадать с образом 0.9.2 (п. 4.15.1), чтобы CRC существующих проектов не менялась. Если регион `FLASH` в скрипте определить нельзя, CRC отключается с предупреждением при Configure (как в п. 4.15.4). Причина: заполнение `--gap-fill` между Flash и секцией вне Flash раздувает образ до сотен мегабайт. `[U]` `(р.1.5)`

### 4.16. Классы диагностических сообщений

4.16.1. Ошибкой Configure (`FATAL_ERROR`) ДОЛЖНЫ завершаться ситуации, при которых сборка заведомо невозможна или неоднозначна: нет yq, файла конфигурации, IOC, пакета STM32Cube, каталога Arduino Core, явного скрипта компоновщика, `hal_conf.h`; неверный формат размера памяти; HAL без CMSIS; ноль или несколько портов FreeRTOS; каталог в `sources` без `CMakeLists.txt`; Arduino без шаблона скрипта; `STM32_YML_PROFILE=list`. Также ошибкой Configure ДОЛЖНЫ завершаться: многоядерный MCU без `mcu_core` и недопустимый `mcu_core` (п. 4.7.8); отсутствующий компонент HAL или FreeRTOS (п. 4.7.9, 4.8.7); ненайденный внешний FreeRTOS (п. 4.8.6); CRC со скриптом stm32-cmake (п. 4.15.2). На стадии post-build ошибкой ДОЛЖЕН завершаться сбой расчёта CRC (п. 4.15.7). `[U]` `(р.1.2)`

4.16.2. Предупреждением (`WARNING`) с продолжением ДОЛЖНЫ сопровождаться: несовпадение или отсутствие версии конфигурации; неизвестный профиль; отсутствующий файл профилей; отсутствующие элементы `sources`, `custom_libraries`, библиотеки и обёртки Arduino; отсутствие `mcu_target`; отсутствие Cppcheck; нераспознанные RAM-области; отключение CRC из-за отсутствия prerequisites; неизвестное значение перечислимого ключа (п. 3.7); семейство вне таблицы портов FreeRTOS (п. 4.4.4); неприменённые heap/stack (п. 4.11.9); секция CRC не найдена в тексте скрипта (п. 4.15.2). `[U]` `(р.1.2)`
...
5.2.1. При backend stm32-cmake toolchain ДОЛЖЕН предоставлять функции stm32-cmake: `stm32_get_chip_info`, `stm32_get_cores`, `stm32_get_memory_info`, `stm32_add_linker_script`, `stm32_print_size_of_target`, `stm32_generate_binary_file`, `stm32_generate_hex_file`, `stm32_generate_srec_file`, а также пакеты CMSIS, HAL, FreeRTOS и цели `STM32::Nano`, `STM32::Nano::FloatPrint`, `STM32::Nano::FloatScan`, `STM32::NoSys`. Цель `STM32::Semihosting` stm32-cmake (ревизия `ecc5acc`) не определяет; её создаёт фреймворк (п. 4.10.2). `[U]` `(р.1.2)`
...
5.6.2. Интерфейс скрипта: `stm32_crc.py <вход.bin> <выход.bin> [предел_байт]` для готового двоичного образа и `stm32_crc.py --elf <вход.elf> --flash <начало>:<длина> --exclude <секция> <выход.bin> [предел_байт]` для формирования образа по п. 4.15.9; выход — файл из 4 байт CRC little-endian; сообщение `[STM32 CRC32] Calculated: 0x…` при успехе, диагностика с префиксом `[CRC ERROR]` и ненулевой код при сбое (п. 4.15.7). `[U]` `(р.1.5)`
...
8.2.1. Автоматические проверки L0, L3–L5 ДОЛЖНЫ выполняться в Docker-образах с зафиксированными версиями: базовый образ по digest, toolchain, CMake, Ninja, yq, stm32-cmake, пакеты STM32Cube (с 0.9.3 — также H5 и H7), FreeRTOS-Kernel для режима `external`, Arduino Core, ETL (`ci/dependencies.lock.json`), QEMU и Renode (`ci/emulation/versions.lock.json`); архивы проверяются по SHA-256 до распаковки. `[U]` `(р.1.2)`
...
8.5.6. Если конвейер GitLab CI добавляется в репозиторий, проверки в GitHub Actions и GitLab CI ДОЛЖНЫ выполняться одинаково: те же Docker-образы, lock-файлы, скрипты `ci/` и манифесты тестов; различаются только описания конвейеров. Состав и результаты проверок для одного коммита ДОЛЖНЫ совпадать. Для 0.9.3 конвейер GitLab CI не требуется (вопрос 10.2.19). `[N]` `(р.1.4)`
...
| TC-22 | (L3) External FreeRTOS из дерева Cube F4 через `-DFREERTOS_PATH` и через окружение | Configure и Generate успешны; связаны `FreeRTOS::ARM_CM4F`, `FreeRTOS::Heap::4`; нет целей `FreeRTOS::STM32::F4::*`. До 0.9.3 сценарии с суффиксом `known-failure` воспроизводили E007. Сценарии: `configure.freertos-external-cmake`, `configure.freertos-external-env` `(р.1.6)` |
...
| TC-25 | Диагностика (diagnostics-toggle, diagnostics-unrecognized-ram); для 0.9.3 — также скрипт stm32-cmake и шаблон F3/G4 с CCMRAM | Переключение вывода свойств и проверки RAM; несовпадение RAM не ошибка; нераспознанная RAM — предупреждение; скрипт stm32-cmake — «не проверялось»; RAM+CCMRAM — равенство `(р.1.2)` |
...
| TC-27 | Артефакты (artifacts-defaults, -all, -map-only, -empty, -unknown, -reconfigure); для 0.9.3 — `srec` и цель с `OUTPUT_NAME` | Команды bin/hex/srec/lss, флаг map; неизвестный пропускается (в 0.9.3 — с предупреждением по п. 3.7); имена `lss`/`map` по `OUTPUT_NAME`; нет остатков `(р.1.2)` |
| TC-28 | Команда CRC (crc-command-generation); для 0.9.3 — CRC со скриптом stm32-cmake и с шаблоном без секции | Post-build команды с секцией и пределом Flash присутствуют/отсутствуют по `crc_enable`; скрипт stm32-cmake — ошибка Configure; шаблон без секции — предупреждение `(р.1.2)` |
...
| TC-41 | (предлагается) `verbose_build` | `CMAKE_VERBOSE_MAKEFILE` ON/OFF; проверки `mcu_core` — TC-54, TC-55 `(р.1.2)` |
...
| TC-43 | (L2) Скрипт CRC напрямую: векторы приложения B; отсутствующий вход; вход больше предела | Совпадение векторов; при сбое — ненулевой код, диагностика, заглушка не записывается. Сценарии: `tests/test_firmware_crc_script.py` `(р.1.6)` |
...
| TC-46 | (L3) Неизвестные значения `system_library`, `cmsis_rtos_api`, элемента `build_artifacts`, `crc_algorithm`; пустые значения тех же ключей | Предупреждение с ключом, значением и фактическим значением по таблице п. 3.7.3; для пустых — без предупреждения; Configure успешен. Сценарии: `configure.enum-unknown-values`, `configure.freertos-unknown-enums`, `configure.artifacts-unknown`, `configure.crc-unknown-algorithm`, `configure.empty-and-null-defaults` `(р.1.6)` |
| TC-47 | (L3) После Configure изменить YAML, затем IOC, затем файл профилей и выполнить сборку | Файлы входят в зависимости перезапуска CMake в `build.ninja`; после изменения каждого файла `ninja -n build.ninja` планирует повторный Configure. Сравнение с Configure в чистой папке не автоматизировано. Сценарии: `configure.configure-depends` `(р.1.6)` |
| TC-48 | (L3) `profiles_file` с ключами верхнего уровня (в том числе базой для `_append`) и встроенной секцией `profiles:` | Ключи вне `profiles:` не влияют на результат; доступны только внешние профили, о встроенных выводится предупреждение (п. 3.4.11). Сценарии: `configure.external-profile-ignores-top-level`, `configure.external-profile` `(р.1.6)` |
| TC-49 | (L3) IOC без `ProjectName`, `HeapSize`, `StackSize` | `auto` (имя каталога), 512, 1024. Сценарии: `configure.ioc-missing-values-defaults` `(р.1.6)` |
| TC-50 | (L3) Размеры `1k`, `1.5K`, `-1`, `0x200` при шаблоне и при явном скрипте; `0`, `1536`, `2K`, `1M` | Ошибка Configure в обоих режимах; допустимые значения приняты. Сценарии: `configure.memory-size-format`, `configure.invalid-memory-size` `(р.1.6)` |
| TC-51 | (L3–L5) Профиль задаёт `compile_options_c`, `compile_options_cxx`, `compile_definitions_c`, `compile_definitions_cxx` и собственный ключ без объявления в корне | Значения применены к командам C/C++; собственный ключ доступен в `CMakeLists.txt`. Сценарии: `configure.profile-only-keys`; профили прошивки `cmsisLibrary`, `cmsisEtl` без объявлений в корне `(р.1.6)` |
| TC-52 | (L4) Сборка с `crc_enable: true` и заниженным `flash_size` | Сборка завершается ошибкой шага CRC (`[CRC ERROR]`), в том числе при повторной сборке. Сценарии: `crc_limit_negative` в `ci/build_firmware_smoke.py` (профиль `success`, `flash_size: 4K`) на каждой паре `(р.1.7)` |
| TC-53 | (предлагается, после 0.9.3, I/T) Проверки выпуска в GitHub Actions и GitLab CI на одном коммите | Совпадают состав уровней, число сценариев, сборок и запусков, итог `(р.1.4)` |
| TC-54 | (L3) H7: одноядерный `STM32H743ZI` без `mcu_core`; двухъядерный `STM32H745ZI` без ядра, с `M7`, с `M4`; `mcu_core: M3`; `mcu_core: M4` для F4 | Одноядерный — ядро `M7` с сообщением, цели `CMSIS::STM32::H743ZI::M7`, `HAL::STM32::H7::M7::*`; двухъядерный без ядра и недопустимые ядра — ошибка со списком; размер RAM/Flash для проверки RAM и CRC — по ядру. Сценарии: `configure.h7-single-core-default`, `configure.h7-dual-core`, `configure.mcu-core-invalid` `(р.1.6)` |
| TC-55 | (L3) H7 `M7` с FreeRTOS из Cube, `cmsis_rtos_api: v2`, `Heap::4` | Цели `FreeRTOS::STM32::H7::M7::ARM_CM7`, `CMSIS::STM32::H7::M7::RTOS_V2`; Generate успешен. Сценарии: `configure.h7-freertos-cube` `(р.1.6)` |
| TC-56 | (L3) H5 `STM32H563ZI`: CMSIS/HAL; FreeRTOS `external` из FreeRTOS-Kernel с `ARM_CM33_NTZ`; `crc_enable: true` с шаблоном | Цели CMSIS/HAL H5; `FreeRTOS::ARM_CM33_NTZ` (для NTZ-порта stm32-cmake не добавляет суффикс `::NON_SECURE`); CRC по п. 4.15.8, 4.15.9. Сценарии: `configure.h5-cmsis-hal-freertos-external` `(р.1.6)` |
| TC-57 | (L4–L5) Сборка минимальной прошивки для H7 (`STM32H743ZI`, `M7`) и H5 (`STM32H563ZI`) на всех парах и запуск в Renode на моделях `h7-smoke`, `h5-smoke` | Непустые ELF/BIN/HEX; проверки ELF и раскладки проходят; в Renode — ожидаемые CPUID и `.data`, TEST_RESULT=PASS и SYS_EXIT_EXTENDED со статусом 0; QEMU не применяется (п. 8.8.5). Сценарии: профили `h7`, `h5` в `tests/firmware/buildonly`; Renode `(р.1.6)` |
| TC-58 | (L3) External FreeRTOS из FreeRTOS-Kernel для F4 с `cmsis_rtos_api: none` и `v2`; без `FREERTOS_PATH`; с порт-компонентом, которого нет в дереве | Успех с целями `FreeRTOS::*`, для `v2` — с `CMSIS::STM32::F4::RTOS_V2` (п. 4.8.8); без пути и без порта в дереве — ошибка Configure. Сценарии: `configure.freertos-external-kernel`, `-no-path`, `-missing-port`, `-missing-component` `(р.1.6)` |
| TC-59 | (L4–L5) Профиль прошивки `freertosExternal`: сценарий `freertosQueue` с FreeRTOS-Kernel (`ARM_CM3`) | PASS в QEMU и Renode на всех парах. Сценарии: профиль `freertosExternal` `(р.1.6)` |
| TC-60 | (L3) Порт FreeRTOS по IOC для F1, F4, G0, C0, U0, H5, L5, U5, WL, WB, H7 с `M7` и `M4`; незнакомое семейство | Значение `freertos_components` по таблице приложения A; незнакомое семейство — предупреждение и `ARM_CM4F`. Сценарии: `configure.freertos-ioc-port-table`, `configure.freertos-ioc-port-unknown-family` `(р.1.6)` |
| TC-61 | (L3) Несуществующий компонент в `hal_components` и в `freertos_components` (Cube и `external`) | Ошибка Configure с именем компонента до стадии Generate. Сценарии: `configure.hal-missing-component`, `configure.freertos-missing-component`, `configure.freertos-external-missing-component` `(р.1.6)` |
| TC-62 | (L3) `heap_size` из YAML, профиля, override и IOC со скриптом stm32-cmake и с явным `linker_script`; значения ручного режима по умолчанию | Предупреждение о неприменённых размерах с размерами stm32-cmake; для значений по умолчанию предупреждения нет. Сценарии: `configure.memory-size-unused-warning`, `configure.ioc-bluepill-defaults`, `configure.ioc-missing-values-defaults` `(р.1.6)` |
| TC-63 | (L4–L5) `STM32H503CB` с шаблоном `.ld.in` и секцией `.checksum`: без секций вне Flash и с инициализированной секцией в резервной SRAM (`0x40036400`) | Промежуточный образ содержит только секции Flash; CRC совпадает с независимым расчётом на хосте и одинакова в обоих вариантах; сборка успешна; в Renode прошивка повторно считает CRC по Flash (CRC_RESULT=PASS) и видит загруженное слово резервной SRAM только в варианте с секцией. Сценарии: профили `h503`, `h503bkp`; независимый расчёт на хосте, расчёт в прошивке в Renode `(р.1.6)` |
| TC-64 | (L4) Образ по п. 4.15.9 для всех профилей прошивочной матрицы | Побайтное совпадение с образом `objcopy --gap-fill` 0.9.2; CRC не меняется. Сценарии: `compare_gap_fill` в `ci/build_firmware_smoke.py` для всех профилей с CRC и `h503` `(р.1.6)` |
| TC-65 | (L3) H5 `STM32H563ZI` со startup STM32CubeH5 1.7.0: скрипт stm32-cmake; шаблон без `_sstack`; явный скрипт с `_sstack`; H7 | Предупреждение п. 4.11.10 в первых двух случаях; в третьем и для H7 — нет. Сценарии: `configure.h5-cmsis-hal-freertos-external`, `configure.h7-single-core-default` `(р.1.6)` |
| TC-66 | (L2–L4) BIN по п. 4.14.2: команда вместо `objcopy -O binary`; ELF с инициализированной секцией в резервной SRAM | В `build.ninja` — `stm32_crc.py --image`, `objcopy -O binary` отсутствует; BIN `h503bkp` равен образу Flash с CRC (1036 байт вместо ~900 МБ); для обычных прошивок BIN побайтно совпадает с прежним. Сценарии: `configure.artifacts-*`, `tests/test_firmware_crc_script.py`, профиль `h503bkp` `(р.1.6)` |
| TC-67 | (L4–L5) Матрица прошивок по целям п. 8.8.8: все профили и повреждённая копия для каждой цели на всех парах | Проверки ELF в пределах FLASH/RAM цели; startup, HAL, версия FreeRTOS и метаданные цели по `expected-metadata.json`; PASS (или ожидаемый отказ) в QEMU, если машина есть, и в Renode. Сценарии: `f030:*` в `ci/firmware_cases.py` `(р.1.8)` |


### 8.8. Предвыпускная стабилизация 0.9.3 `(р.1.2)`

8.8.1. До выпуска 0.9.3 в `ci/dependencies.lock.json` и образ CI ДОЛЖНЫ быть добавлены STM32CubeH7 1.13.0 (подмодули CMSIS device, HAL и FreeRTOS), STM32CubeH5 1.7.0 (подмодули CMSIS device и HAL; FreeRTOS в пакете нет) и FreeRTOS-Kernel 11.3.1 (каталог образа `/opt/FreeRTOS-Kernel/11.3.1`). Пакеты Cube устанавливаются тем же минимальным набором файлов, что и для остальных семейств (п. 8.2.1). Версии выбраны по вопросу 10.2.18. `[N]` `(р.1.3)`

8.8.2. H7 ДОЛЖЕН проверяться на уровне L3 сценариями TC-54 и TC-55: одноядерный MCU без `mcu_core`, двухъядерный MCU без ядра и с каждым ядром, недопустимое ядро, FreeRTOS из Cube с обёрткой CMSIS-RTOS. `[N]` `(р.1.2)`

8.8.3. H5 ДОЛЖЕН проверяться на уровне L3 сценарием TC-56: CMSIS/HAL, FreeRTOS через `external` с портом `ARM_CM33_NTZ`, поведение CRC по п. 4.15.8. `[N]` `(р.1.2)`

8.8.4. Режим `external` ДОЛЖЕН проверяться на двух раскладках — дереве FreeRTOS из пакета Cube и FreeRTOS-Kernel — на уровне L3 (TC-22, TC-58, включая `cmsis_rtos_api: v2`) и на уровне L4–L5 профилем прошивки `freertosExternal` (TC-59). `[N]` `(р.1.2)`

8.8.5. Для H7 и H5 ДОЛЖНА выполняться сборка прошивки на всех парах и её запуск в Renode на минимальных моделях (`tests/firmware/renode/h7-smoke.repl`, `h5-smoke.repl`: ядро, NVIC, память и RAM-заглушки регистров SystemInit) со статусом runnable (TC-57, п. 8.4.6). QEMU для них не применяется: в нём нет машины с Cortex-M7 или Cortex-M33 и Flash по адресу `0x08000000`. Периферия и тактирование в этих моделях не проверяются. `[N]` `(р.1.6)`

8.8.6. Проблема, выявленная проверками п. 8.8.2–8.8.5, ДОЛЖНА устраняться до выпуска 0.9.3 с фиксацией новой ревизией ТЗ. Тест-кейсы TC-22, TC-54 … TC-59 входят в критерий п. 8.6.1(г) для версии 0.9.3 независимо от пометки «(предлагается)»; отказ от проверки допускается только решением заказчика с записью в разделе 10. `[N]` `(р.1.2)`

8.8.7. Формирование образа CRC по п. 4.15.9 ДОЛЖНО проверяться до выпуска 0.9.3 тест-кейсами TC-63 и TC-64; они входят в критерий п. 8.6.1(г) независимо от пометки «(предлагается)». После внесения доработок 0.9.3 и перед выпуском ДОЛЖНЫ быть повторно выполнены локально все проверки L0–L5 (п. 8.6.1(а)) на итоговом коммите ветки выпуска. `[N]` `(р.1.5)`

8.8.8. До выпуска 0.9.3 матрица прошивок (L4–L5) ДОЛЖНА собирать полный набор профилей тестового примера `tests/firmware/semihosting` (13 профилей и повреждённая копия) на всех парах инструментов для целей: F1 `STM32F103C8T6` (опорная), F0 `STM32F030R8T6`, F4 `STM32F411CEU6` и `STM32F401CCU6`, G4 `STM32G431CBU6` и `STM32G474CEU6`, F7 `STM32F746ZG`. Прошивка ДОЛЖНА запускаться в QEMU, если в нём есть машина с совместимым ядром и картой памяти (F0 и F1 — `netduino2`, F4 — `netduinoplus2`), и в Renode на минимальной модели с ядром цели (`tests/firmware/renode/<цель>-smoke.repl`). Цель, для которой не удалось настроить ни один эмулятор, записывается в раздел 10 с решением заказчика. Цели добавляются по одному семейству, каждое — с полной локальной проверкой L3–L5; состояние — в приложении E. `[N]` `(р.1.8)`
...
| 3.4.11 | PRF: только внешние профили, предупреждение о встроенных `(р.1.6)` | TC-48 |
...
| 4.4.1–4.4.3, 4.4.5 | UTL: `stm32_yml_parse_ioc_file`; CFG: IOC-ветка | TC-11, TC-21 |
| 4.4.4 ⚠ | CFG: таблица портов FreeRTOS | TC-21, TC-60 |
...
| 4.7.3 ⚠ | FWK: CMSIS с ядром | TC-04, TC-17, TC-54 |
| 4.7.4, 4.7.5 | FWK: HAL/LL | TC-17, TC-18 |
| 4.7.6 | DIA: проверка `hal_conf.h` | TC-18 |
| 4.7.7 | FWK, LNK | TC-16, TC-37 |
| 4.7.8 ⚠ | YML, FWK: `stm32_get_cores`, выбор ядра | TC-54, TC-55 |
| 4.7.9 ⚠ | FWK: проверка целей HAL | TC-61 |
| 4.8.1, 4.8.2, 4.8.4, 4.8.5 | FWK: FreeRTOS | TC-19, TC-20, TC-21, TC-40 |
| 4.8.3 ⚠ | FWK: обёртки CMSIS-RTOS с ядром, предупреждение по п. 3.7 | TC-19, TC-46, TC-55 |
| 4.8.6 ⚠ | FWK: `find_package(FreeRTOS …)` без компонента семейства (E007) | TC-22, TC-56, TC-58, TC-59 |
| 4.8.7 ⚠ | FWK: проверка целей FreeRTOS | TC-61 |
| 4.8.8 | FWK; DOC | TC-58 |
...
| 4.10.4 | stm32-cmake: `STM32::Nano::FloatPrint`; DOC | I |
...
| 4.11.9 ⚠ | LNK: предупреждение о неприменённых heap/stack | TC-62 |
| 4.11.10 | LNK: предупреждение об отсутствующем `_sstack` `(р.1.6)` | TC-65 |
...
| 4.13.4 ⚠ | DIA: проверка RAM с CCRAM/RAM_SHARE и ядром | TC-25, TC-54 |
| 4.14.1, 4.14.3 | PB; UTL: `stm32_yml_generate_lss_file` | TC-27, TC-33 |
| 4.14.2 ⚠ | PB, UTL: `srec`, BIN из секций FLASH, предупреждение о неизвестном элементе `(р.1.6)` | TC-27, TC-46, TC-66 |
| 4.15.1 ⚠, 4.15.9 ⚠ | PB; CRC: образ по секциям ELF в регионе `FLASH` `(р.1.5)` | TC-28, TC-36, TC-63, TC-64 |
| 4.15.3, 4.15.5 | PB; CRC | TC-28, TC-36, TC-43 |
| 4.15.2 ⚠ | PB: проверка секции CRC на Configure | TC-28 |
| 4.15.4 ⚠ | PB: `stm32_get_memory_info` с ядром | TC-28, TC-54 |
| 4.15.6 ⚠ | PB, DOC: принудительный `STM32_HW_DEFAULT` (E004) | TC-46 |
| 4.15.7 ⚠ | CRC: ненулевой код вместо `graceful_exit` (E006) | TC-43, TC-52 |
| 4.15.8 ⚠ | CRC, PB; DOC, CHANGELOG `(р.1.5)` | TC-56, TC-63 |
...
| 5.2.1–5.2.3 | YML, LNK, PB, DIA, FWK | TC-04, TC-27, TC-30, TC-33, TC-54 |
...
| 8.5.6 | CI: `.github/workflows/`; конвейер GitLab CI — после 0.9.3 `(р.1.4)` | TC-53 |
| 8.6.1, 8.6.2 | Процесс выпуска | I |
| 8.8.1 | CI: `dependencies.lock.json`, Dockerfile | TC-01 |
| 8.8.2, 8.8.3 | CI: `tests/cases.json` | TC-54, TC-55, TC-56 |
| 8.8.4 | CI: `tests/cases.json`, профиль `freertosExternal` | TC-22, TC-58, TC-59 |
| 8.8.5 | CI: `build_firmware_smoke.py`, `run_renode_smoke.py`, модели Renode H7/H5 `(р.1.6)` | TC-57 |
| 8.8.6 | Процесс выпуска | I |
| 8.8.7 | Процесс выпуска; CI `(р.1.5)` | TC-63, TC-64, I |
| 8.8.8 | CI: `firmware_cases.py`, `build_firmware_smoke.py`, модели Renode целей `(р.1.8)` | TC-67 |
...
| 10.2.8 | External FreeRTOS (E007): поддерживать режим или явно отклонять? | Закрыт | Поддерживать. Причина E007 установлена по исходникам stm32-cmake: компонент семейства переключает поиск на Cube. Исправление — поиск без компонента семейства; проверка на дереве Cube и FreeRTOS-Kernel, включая H5, где Cube не содержит FreeRTOS `(р.1.2)` | 4.8.6–4.8.8, 8.8.4, TC-22, TC-58, TC-59 |
| 10.2.9 | Отсутствие обёртки ядра Arduino — предупреждение или ошибка? | Закрыт | В 0.9.3 поведение 0.9.2 сохраняется: отсутствие обёртки ядра — предупреждение. Подключение `CMakeLists.txt` из `cores` и `libraries` Arduino_Core_STM32 напрямую и новые ключи — в 0.10.x вместе с переходом на CMake ≥ 3.21, после проверок на прототипах конфигураций со сборкой прошивок. Результаты прототипа — п. 10.3.5 `(р.1.5)` | 4.9.4, 10.3.5, TC-30 |
...
| 10.2.17 | Как формировать промежуточный образ для CRC на STM32H5, чтобы он не раздувался заполнением между Flash и областями вне Flash? Варианты: исключать секции с адресами вне Flash; вырезать диапазон Flash по скрипту компоновщика; считать CRC по загружаемым сегментам ELF | Закрыт | Образ строится из секций ELF с адресом загрузки в регионе `FLASH` скрипта. Причина раздувания — секции вне Flash, а не семейство: `STM32H503CB` (плата WeAct `STM32H503CBT6`; `STM32H503C8` в базе stm32-cmake нет) со штатным скриптом Cube даёт образ 1128 байт, инициализированная секция в резервной SRAM — 896 МиБ и заглушку E006. Прототип проверен вне репозитория: побайтное совпадение с образом `objcopy` для 72 прошивок матрицы, верная CRC для H503CB с резервной SRAM. Перед выпуском — повтор всех локальных проверок `(р.1.5)` | 4.15.1, 4.15.8, 4.15.9, 5.6.2, 8.8.7, TC-63, TC-64 |
| 10.2.18 | Какие версии STM32CubeH7, STM32CubeH5 и FreeRTOS-Kernel закрепить в lock-файле и какие MCU H7/H5 взять для сценариев (предложены `STM32H743ZI`, `STM32H745ZI`, `STM32H563ZI`)? | Закрыт | Последние теги на 26.09.2026: STM32CubeH7 `v1.13.0`, STM32CubeH5 `v1.7.0`, FreeRTOS-Kernel `V11.3.1`; в lock-файле — коммиты тегов. MCU приняты: все три есть в базе stm32-cmake `ecc5acc`, у `STM32H745ZI` ядра `M7` и `M4`, у `STM32H743ZI` — `M7`; порт `ARM_CM33_NTZ` поддерживается. Проверено установкой в образ и `verify.py` `(р.1.3)` | 8.8.1, TC-54 … TC-59, приложение F |
| 10.2.19 | Входит ли GitLab CI в приёмку 0.9.3? Реквизиты и п. 8.5.6 называют GitLab CI, но конвейера в репозитории нет; п. 8.6.1(б) допускает проверку в GitHub или GitLab; TC-53 помечен «(предлагается)» | Закрыт | Нет. Версия 0.9.3 принимается по GitHub Actions; конвейер GitLab CI и TC-53 — после 0.9.3 `(р.1.4)` | Реквизиты, 8.5.6, 8.6.1, TC-53 |
...
10.3.5. Прототип прямого подключения Arduino Core (26.09.2026; Arduino_Core_STM32 2.12.0, CMake 3.28.3, GCC 14.2.1, плата `BLUEPILL_F103C8`): проект подключает variant, `cores/arduino`, `libraries/SrcWrapper` и отдельно `libraries/Wire` через `add_subdirectory`; цели `board`, `base_config`, `user_settings` создают модули Core `environment`, `boards_db`, `overall_settings`, `set_base_arduino_config`. Сборка успешна (13 144 байт Flash без `Wire`, 19 260 байт с `Wire`); запуск не проверялся, каталог Core не изменяется, загрузок нет. Условия: CMake ≥ 3.21 (`cmake_minimum_required` в `CMakeLists.txt` Core; с CMake 3.19.8 Configure прерывается); `set_board()` не вызывается, так как перегенерирует `boards_db.cmake` в каталоге Core; `CMSIS6_PATH` задаётся явно вместо загрузки `ensure_core_deps()` (для F1 подошли заголовки CMSIS из Cube F1); без `overall_settings()` (newlib-nano, `-Os`) образ не помещается во Flash. Кандидат для 0.10.x. `(р.1.5)`
...
| `mcu_core` | `CFG-MCU-CORE` | единственное ядро MCU по stm32-cmake (H7 — `M7`, WB и MP1 — `M4`), иначе пусто; многоядерный без ядра — ошибка `(р.1.2)` | как отсутствие | 4.7.8 |
...
| `freertos_components` | `CFG-FREERTOS-COMPONENTS` | по IOC: F1/F2/L1/WL(M4) — `ARM_CM3`; F3/F4/G4/L4/WB/MP1/H7(M4) — `ARM_CM4F`; F0/G0/L0/C0/U0/WL(M0PLUS) — `ARM_CM0`; F7/H7(M7) — `ARM_CM7`; H5/L5/U5 — `ARM_CM33_NTZ`; иное — предупреждение и `ARM_CM4F`; плюс `Heap::4`. Вручную — `[]` `(р.1.2)` | как отсутствие | 4.4.4 |
...
| `use_newlib_nano` | `CFG-USE-NEWLIB-NANO` | `false`; float в printf/scanf — `STM32::Nano::FloatPrint`/`FloatScan` через `link_libraries` `(р.1.2)` | `false` | 4.10.1 |
...
| `build_artifacts` | `CFG-BUILD-ARTIFACTS` | `[]`; элементы `bin`, `hex`, `srec`, `map`, `lss`; неизвестный — предупреждение `(р.1.2)` | только ELF и размер | 4.14.2 |
...
| Состав промежуточного образа CRC | секции с содержимым и адресом загрузки в регионе `FLASH`, кроме секции CRC; промежутки `0xFF` `(р.1.5)` | 4.15.9 |
...
Перечень доработок кода и документации, необходимых для выполнения требований версии 0.9.3 (пункты с пометкой ⚠ в разделе 9). Состояние указано на `main` @ `7246a2c`; строки с пометкой `(р.1.3)` уточнены на `e39863d`. `(р.1.1)`

### E.1. Доработки кода

Состояние на ветке `claude/release-0.9.3` (коммиты `4cd3404`…`97682e3`); до слияния и выпуска исправления имеют статус «подготовлено». Исходное описание отклонений 0.9.2 сохранено в ревизиях 1.1–1.5 и в errata. `(р.1.6)`
...
| 3.4.8, 3.4.11 | Из `profiles_file` только `profiles:`; предупреждение о встроенных профилях | Реализовано: `d8708b4`, `489bd61`; TC-48 `(р.1.6)` | Вопрос 10.2.16 |
| 3.4.9 | Ключи только из профиля применяются без объявления в корне | Реализовано: `d8708b4`; TC-51; обход удалён из прошивок `(р.1.6)` | E008 |
| 3.6.5 | `CMAKE_CONFIGURE_DEPENDS` для YAML, IOC, `profiles_file` | Реализовано: `d8708b4`; TC-47 `(р.1.6)` | Вопрос 10.2.5 |
| 3.7, 4.1.6, 4.8.3, 4.10.2, 4.14.2, 4.15.6 | Предупреждение и фактическое значение для неизвестных значений перечислений | Реализовано: `4cd3404` (`crc_algorithm`), `b9a6cd3`; TC-46 `(р.1.6)` | Вопросы 10.2.2, 10.2.11; E004 |
| 4.2.3 | Текст предупреждения: версия конфигурации рекомендуемая | Реализовано: `d8708b4` `(р.1.6)` | Вопрос 10.2.6 |
| 4.4.6 | Значения ручного режима при неполном IOC | Реализовано: `d8708b4`; TC-49 `(р.1.6)` | Вопрос 10.2.7 |
| 4.11.4, 4.11.8 | Проверка формата размеров всегда; текст ошибки без `1.5K` | Реализовано: `d8708b4`; TC-50 `(р.1.6)` | Вопрос 10.2.10; E003 |
| 4.11.9 | Предупреждение о неприменённых heap/stack | Реализовано: `d8708b4`; TC-62 `(р.1.6)` | Анализ stm32-cmake |
| 4.11.10 | Предупреждение об отсутствующем `_sstack` | Реализовано: `2625f19`; TC-65 `(р.1.6)` | Проверка примеров `demo-stm32-cmake/stm32h5xx` |
| 4.15.2, 4.15.7–4.15.9 | Проверка секции CRC; сбой CRC — провал сборки; образ по секциям Flash | Реализовано: `4cd3404`; TC-43, TC-52, TC-63, TC-64 `(р.1.7)` | E006; вопросы 10.2.12, 10.2.17 |
| 4.8.6–4.8.8 | External FreeRTOS: поиск без компонента семейства; обёртка CMSIS-RTOS; `portasm.c` порта | Реализовано: `3c6bfa3`; `portasm.c` — шаг F0; TC-22, TC-56, TC-58, TC-59, TC-67 `(р.1.8)` | E007; вопрос 10.2.8 |
| 8.8.8 | Матрица прошивок по семействам | F0 `STM32F030R8T6`: QEMU `netduino2` (ядро M3), Renode `f030-smoke` (cortex-m0) — реализовано. F4, G4, F7 — в работе `(р.1.8)` | Решение заказчика |
| 4.4.4 | Таблица портов FreeRTOS; предупреждение для незнакомого семейства | Реализовано: `3c6bfa3`; TC-60 `(р.1.6)` | Анализ stm32-cmake |
| 4.7.3, 4.7.8, 4.8.3, 4.13.4, 4.15.4 | Выбор и проверка `mcu_core`; ядро в целях CMSIS-RTOS и в `stm32_get_memory_info` | Реализовано: `3c6bfa3`; TC-54, TC-55 `(р.1.6)` | Анализ stm32-cmake |
| 4.7.9, 4.8.7 | Проверка целей компонентов HAL и FreeRTOS на Configure | Реализовано: `3c6bfa3`; TC-61 `(р.1.6)` | Анализ stm32-cmake |
| 4.13.4 | «Не проверялось» для скрипта stm32-cmake; учёт CCRAM и RAM_SHARE | Реализовано: `3c6bfa3`; TC-54, TC-56 `(р.1.6)` | Анализ stm32-cmake |
| 4.14.2 | Артефакт `srec`; BIN из секций FLASH; имена `lss`/`map` как в 0.9.2 | Реализовано: `b9a6cd3` (`srec`), `2625f19` (BIN); имена без изменений по решению заказчика; TC-27, TC-66 `(р.1.6)` | Анализ stm32-cmake; проверка H503 |
| 4.9.4 | Обёртка ядра Arduino | Без изменений в 0.9.3 (вопрос 10.2.9) | Вопрос 10.2.9 |
| 4.7.6 | Подсказка о `hal_conf.h` | Реализовано: `d8708b4` (фактическое имя файла конфигурации) `(р.1.6)` | Код DIA |

### E.2. Доработки документации

| Пункты ТЗ | Требуется | Состояние |
| :--- | :--- | :--- |
| 3.6.6 | Рекомендация удалять кэш и перенастраивать после изменения YAML в 0.9.2 | Правило для 0.9.2 в `troubleshooting.md` и errata E001 `(р.1.7)` |
| 3.7.3, 4.15.6 | Таблица перечислимых ключей; описание алгоритма CRC в карточке `CFG-CRC-ALGORITHM` | Карточки перечислимых ключей описывают предупреждение и фактическое значение `(р.1.6)` |
| 4.15.8, 4.15.9 | Формирование образа CRC и снятие ограничения H5 в карточке `CFG-CRC-ENABLE` и CHANGELOG | Описано (`886a08c`) `(р.1.6)` |
...
| 8.2.1 | Состав образов | Исправлено в `e39863d`: `docs/ru/testing.md` и `docs/en/testing.md` относят QEMU и Renode к образу эмуляции и перечисляют H5, H7 и FreeRTOS-Kernel `(р.1.3)` |
| 8.5.6 | Конвейер GitLab CI | Отсутствует; для 0.9.3 не требуется (вопрос 10.2.19) `(р.1.4)` |
| Все | Карточки `CFG-*`, errata и индекс — к версии 0.9.3 | Карточки 0.9.2 дополнены отметками «Изменение в 0.9.3» со ссылкой на коммит и пункты ТЗ; errata E004, E006, E007, E008 — «исправление подготовлено»; индекс указывает новые сценарии `(р.1.6)` |
| 4.7.8 | Карточка `CFG-MCU-CORE`: ядро обязательно для многоядерных MCU, для H7 выбирается автоматически | Описано (`1bb1568`) `(р.1.6)` |
| 4.8.6 | Карточка `CFG-FREERTOS-VERSION`: FreeRTOS-Kernel, пакеты Cube без FreeRTOS (H5, U5) | Описано (`1bb1568`) `(р.1.6)` |
| 4.10.2, 4.10.4 | Комментарий в `stm32_yml_utils.cmake` о `STM32::Semihosting`; описание FloatPrint/FloatScan | Комментарий исправлен; FloatPrint/FloatScan описаны в карточке `CFG-USE-NEWLIB-NANO` `(р.1.7)` |
...
| E007 | 4.8.6 | Исправление в 0.9.3 (поиск без компонента семейства); регрессии TC-22, TC-58, TC-59 `(р.1.2)` |
...
| STM32Cube | F0 1.11.6, F1 1.8.7, F3 1.11.5, F4 1.28.3, F7 1.17.3, G4 1.6.3, H5 1.7.0, H7 1.13.0 (п. 8.8.1) `(р.1.3)` |
| FreeRTOS-Kernel | 11.3.1, `/opt/FreeRTOS-Kernel/11.3.1` (п. 8.8.1) `(р.1.3)` |
| Arduino Core STM32 / ETL | 2.12.0 / 20.47.1 |
| QEMU / Renode | 11.0.0 (`netduino2`, `netduinoplus2`) / 1.16.1 (`f103-smoke`, `f030-smoke`, `h7-smoke`, `h5-smoke`) `(р.1.8)` |

### F.2. Объём проверок

| Уровень | На `7246a2c` (0.9.2) | На ветке `claude/release-0.9.3` `(р.1.8)` |
| :--- | :--- | :--- |
| L2 | — | 42 unittests |
| L3 | 115 сценариев × 6 пар = 690 запусков; 15 многошаговых, 18 с ожидаемой ошибкой | 139 сценариев × 6 пар = 834 запуска; 26 многошаговых, 22 с ожидаемой ошибкой |
| L4 | 12 профилей × 6 пар = 72 сборки | (13 профилей × 2 цели F103, F030 + 4 профиля H7/H5) × 6 пар = 180 сборок |
| L5 | 78 запусков QEMU и 78 запусков Renode (включая повреждённый ELF на каждую пару) | 168 запусков QEMU (F103, F030) и 192 запуска Renode (F103, F030, H7/H5), включая повреждённый ELF на каждую цель и пару |
