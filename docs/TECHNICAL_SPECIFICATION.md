...
| **Ревизия** | 1.5 (черновик для согласования) |
...
4.8.8. Обёртки CMSIS-RTOS (п. 4.8.3) ДОЛЖНЫ работать и с внешним FreeRTOS: исходники обёртки берутся из пакета CMSIS семейства, заголовки FreeRTOS — из целей внешнего FreeRTOS. Совместимость подтверждается до выпуска 0.9.3 (п. 8.8.4); если она не подтверждена, сочетание `external` с `v1`/`v2` ДОЛЖНО отклоняться ошибкой Configure с рекомендацией `cmsis_rtos_api: none`, а ограничение описывается в документации. `[N]` `(р.1.2)`
...
4.14.2. Элементы `build_artifacts`: `bin`, `hex` и `srec` ДОЛЖНЫ добавлять преобразование ELF функциями toolchain (`stm32_generate_binary_file`, `stm32_generate_hex_file`, `stm32_generate_srec_file`), `lss` — листинг `objdump -h -S`, `map` — по п. 4.5.7. Имена файлов `lss` и `map` ДОЛЖНЫ строиться так же, как у функций stm32-cmake: из свойства `OUTPUT_NAME` цели, а при его отсутствии — из имени цели. Неизвестный элемент — по п. 3.7 (в 0.9.2 пропускается без предупреждения); пустой список не отключает основную цель. `[U]` `(р.1.2)`
...
| TC-22 | External FreeRTOS из дерева Cube F4 через `-DFREERTOS_PATH` и через окружение (сценарии `freertos-external-cmake-known-failure`, `freertos-external-env-known-failure` переименовываются без суффикса `known-failure`) | До 0.9.3 — воспроизведение E007. Для 0.9.3: Configure и Generate успешны; связаны `FreeRTOS::ARM_CM4F`, `FreeRTOS::Heap::4`; нет целей `FreeRTOS::STM32::F4::*` `(р.1.2)` |
...
| TC-54 | (предлагается, L3, до выпуска 0.9.3) H7: одноядерный `STM32H743ZI` без `mcu_core`; двухъядерный `STM32H745ZI` без ядра, с `M7`, с `M4`; `mcu_core: M3`; `mcu_core: M4` для F4 | Одноядерный — ядро `M7` с сообщением, цели `CMSIS::STM32::H743ZI::M7`, `HAL::STM32::H7::M7::*`; двухъядерный без ядра и недопустимые ядра — ошибка со списком; размер RAM/Flash для проверки RAM и CRC — по ядру `(р.1.2)` |
| TC-55 | (предлагается, L3, до выпуска 0.9.3) H7 `M7` с FreeRTOS из Cube, `cmsis_rtos_api: v2`, `Heap::4` | Цели `FreeRTOS::STM32::H7::M7::ARM_CM7`, `CMSIS::STM32::H7::M7::RTOS_V2`; Generate успешен `(р.1.2)` |
| TC-56 | (предлагается, L3, до выпуска 0.9.3) H5 `STM32H563ZI`: CMSIS/HAL; FreeRTOS `external` из FreeRTOS-Kernel с `ARM_CM33_NTZ`; `crc_enable: true` с шаблоном | Цели CMSIS/HAL H5; `FreeRTOS::ARM_CM33_NTZ` (для NTZ-порта stm32-cmake не добавляет суффикс `::NON_SECURE`); CRC по п. 4.15.8, 4.15.9 `(р.1.5)` |
| TC-57 | (предлагается, L4 build-only, до выпуска 0.9.3) Сборка минимальной прошивки для H7 (`M7`) и H5 на всех парах | Непустые ELF/BIN/HEX; проверки ELF и раскладки проходят; статус build-only `(р.1.2)` |
| TC-58 | (предлагается, L3, до выпуска 0.9.3) External FreeRTOS из FreeRTOS-Kernel для F4 с `cmsis_rtos_api: none` и `v2`; без `FREERTOS_PATH`; с порт-компонентом, которого нет в дереве | Успех с целями `FreeRTOS::*`; для `v2` — успех либо ошибка по п. 4.8.8; без пути и без порта — ошибка Configure `(р.1.2)` |
| TC-59 | (предлагается, L4–L5, до выпуска 0.9.3) Профиль прошивки `freertosExternal`: сценарий `freertosQueue` с FreeRTOS-Kernel (`ARM_CM3`) | PASS в QEMU и Renode на всех парах `(р.1.2)` |
| TC-60 | (предлагается) Порт FreeRTOS по IOC для F1, F4, G0, C0, U0, H5, L5, U5, WL, WB, H7 с `M7` и `M4`; незнакомое семейство | Значение `freertos_components` по таблице приложения A; незнакомое семейство — предупреждение и `ARM_CM4F` `(р.1.2)` |
| TC-61 | (предлагается) Несуществующий компонент в `hal_components` и в `freertos_components` (Cube и `external`) | Ошибка Configure с именем компонента до стадии Generate `(р.1.2)` |
| TC-62 | (предлагается) `heap_size` из YAML, профиля, override и IOC со скриптом stm32-cmake и с явным `linker_script`; значения ручного режима по умолчанию | Предупреждение о неприменённых размерах с размерами stm32-cmake; для значений по умолчанию предупреждения нет `(р.1.2)` |
| TC-63 | (предлагается, L4, до выпуска 0.9.3) `STM32H503CB` с шаблоном `.ld.in` и секцией `.checksum`: без секций вне Flash и с инициализированной секцией в резервной SRAM (`0x40036400`) | Промежуточный образ содержит только секции Flash; CRC совпадает с независимым расчётом на хосте и одинакова в обоих вариантах; сборка успешна `(р.1.5)` |
| TC-64 | (предлагается, L4, до выпуска 0.9.3) Образ по п. 4.15.9 для всех профилей прошивочной матрицы | Побайтное совпадение с образом `objcopy --gap-fill` 0.9.2; CRC не меняется `(р.1.5)` |
...
8.8.5. Для H7 и H5 ДОЛЖНА выполняться сборка прошивки со статусом build-only (TC-57): модели QEMU и Renode для этих семейств в окружении отсутствуют (п. 8.4.6). `[N]` `(р.1.2)`
...
| 4.14.2 ⚠ | PB: `srec`, `OUTPUT_NAME`, предупреждение о неизвестном элементе | TC-27, TC-46 |
...
| 8.8.5 | CI: `firmware_matrix.py` | TC-57 |
...
| 4.15.7–4.15.9 | Сбой CRC — провал сборки; образ по секциям Flash | `graceful_exit` пишет `0x00000000`, код 0; образ через `objcopy --gap-fill`. Прототип формирования по секциям ELF проверен вне репозитория (72 прошивки F103 — побайтное совпадение; `STM32H503CB` с резервной SRAM — верная CRC) `(р.1.5)` | E006; вопросы 10.2.12, 10.2.17 |
| 4.8.6–4.8.8 | External FreeRTOS: поиск без компонента семейства; проверка с CMSIS-RTOS | Ошибка Generate. Пробный Configure на `e39863d` (GCC 14.2.1, CMake 3.28.3): `STM32H563ZI` с FreeRTOS-Kernel — ошибка `find_package(FreeRTOS)`, поиск по семейству `STM32H5` | E007; вопрос 10.2.8 `(р.1.3)` |
| 4.4.4 | Таблица портов FreeRTOS для C0, U0, H5, L5, U5, WL и ядер H7/WL; предупреждение для незнакомого семейства | Незнакомое семейство молча получает `ARM_CM4F` | Анализ stm32-cmake `(р.1.2)` |
| 4.7.3, 4.7.8, 4.8.3, 4.13.4, 4.15.4 | Выбор и проверка `mcu_core`; ядро в целях CMSIS-RTOS и в `stm32_get_memory_info` | Ядро не выбирается; H7 без `mcu_core` ссылается на несуществующую цель; обёртка RTOS и запросы памяти без ядра. Пробный Configure на `e39863d` (GCC 14.2.1, CMake 3.28.3): `STM32H743ZI` и `STM32H745ZI` без ядра — ошибка Generate (цель CMSIS); `STM32H743ZI` с `M7` и `STM32H745ZI` с `M4` — успех; `M7` + FreeRTOS из Cube + `cmsis_rtos_api: v2` — нет цели `CMSIS::STM32::H7::RTOS_V2`; `STM32H563ZI` с CMSIS/HAL — успех | Анализ stm32-cmake `(р.1.3)` |
| 4.7.9, 4.8.7 | Проверка целей компонентов HAL и FreeRTOS на Configure | Ошибка только на Generate | Анализ stm32-cmake `(р.1.2)` |
| 4.11.9 | Предупреждение о неприменённых heap/stack | Значения молча игнорируются | Анализ stm32-cmake `(р.1.2)` |
| 4.13.4 | «Не проверялось» для скрипта stm32-cmake; учёт CCRAM и RAM_SHARE | Выводится равенство без проверки; CCMRAM даёт ложное превышение | Анализ stm32-cmake `(р.1.2)` |
| 4.14.2 | Артефакт `srec`; имена `lss`/`map` по `OUTPUT_NAME` | Нет `srec`; имена по имени цели | Анализ stm32-cmake `(р.1.2)` |
| 4.15.2 | Проверка секции CRC при Configure | Только напоминание | Анализ stm32-cmake `(р.1.2)` |
| 4.9.4 | Обёртка ядра Arduino | Предупреждение при отсутствии; без изменений в 0.9.3 (вопрос 10.2.9) `(р.1.5)` | Вопрос 10.2.9 |
...
| 4.15.8, 4.15.9 | Формирование образа CRC и снятие ограничения H5 в карточке `CFG-CRC-ENABLE` и CHANGELOG | Не описано `(р.1.5)` |
...
| 4.7.8 | Карточка `CFG-MCU-CORE`: ядро обязательно для многоядерных MCU, для H7 выбирается автоматически | Карточка утверждает, что параметр требуется не для каждого MCU, без упоминания H7 `(р.1.2)` |
| 4.8.6 | Карточка `CFG-FREERTOS-VERSION`: FreeRTOS-Kernel, пакеты Cube без FreeRTOS (H5, U5) | Описан только известный отказ `(р.1.2)` |
| 4.10.2, 4.10.4 | Комментарий в `stm32_yml_utils.cmake` о `STM32::Semihosting`; описание FloatPrint/FloatScan | Комментарий утверждает, что цель определяют оба toolchain-файла; stm32-cmake её не определяет `(р.1.2)` |
