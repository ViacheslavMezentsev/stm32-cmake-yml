...
| **Ревизия** | 1.2 (черновик для согласования) |
...
8.8.1. До выпуска 0.9.3 в `ci/dependencies.lock.json` и образ CI ДОЛЖНЫ быть добавлены STM32CubeH7, STM32CubeH5 (минимальный набор файлов CMSIS/HAL, как для остальных семейств) и FreeRTOS-Kernel; версии выбираются при добавлении (вопрос 10.2.18). `[N]` `(р.1.2)`
...
| 10.2.18 | Какие версии STM32CubeH7, STM32CubeH5 и FreeRTOS-Kernel закрепить в lock-файле и какие MCU H7/H5 взять для сценариев (предложены `STM32H743ZI`, `STM32H745ZI`, `STM32H563ZI`)? | Открыт | Решается при добавлении зависимостей до выпуска 0.9.3 `(р.1.2)` | 8.8.1, TC-54 … TC-59 |
...
| 4.8.6–4.8.8 | External FreeRTOS: поиск без компонента семейства; проверка с CMSIS-RTOS | Ошибка Generate | E007; вопрос 10.2.8 `(р.1.2)` |
...
| 4.7.3, 4.7.8, 4.8.3, 4.13.4, 4.15.4 | Выбор и проверка `mcu_core`; ядро в целях CMSIS-RTOS и в `stm32_get_memory_info` | Ядро не выбирается; H7 без `mcu_core` ссылается на несуществующую цель; обёртка RTOS и запросы памяти без ядра | Анализ stm32-cmake `(р.1.2)` |
...
| STM32Cube | F0 1.11.6, F1 1.8.7, F3 1.11.5, F4 1.28.3, F7 1.17.3, G4 1.6.3; до выпуска 0.9.3 добавляются H5 и H7 (п. 8.8.1) `(р.1.2)` |
| FreeRTOS-Kernel | добавляется до выпуска 0.9.3 (п. 8.8.1) `(р.1.2)` |
