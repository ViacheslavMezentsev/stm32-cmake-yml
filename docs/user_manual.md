...
[Документация](ru/index.md) → Руководство · [Reference RU](ru/reference/0.9.2/index.md) · [Reference EN](en/reference/0.9.2/index.md) · [Errata](ru/errata/index.md)

Руководство описывает сценарии. Точные значения, defaults и ограничения версии
проверяйте в справочнике и errata; примеры здесь не заменяют контракт опций.
...
**Опции с отдельным аргументом** распознаются: следующий за ними токен переносится
без изменений, даже если он не начинается с дефиса.

```yaml
compile_options:
  - "--param max-inline-insns-single=500"   # два токена, значение не портится
  - "-include config_prefix.h"
```

Список таких опций: `--param`, `-include`, `-imacros`, `-isystem`, `-iquote`,
`-idirafter`, `-x`, `-Xlinker`, `-Xpreprocessor`, `-Xassembler`, `-u`, `-T`,
`-MT`, `-MF`, `-MQ`, `-aux-info`, `-specs`, `-D`, `-U`, `-I`, `-L`.
Флаг и его значение можно записывать как одной строкой, так и двумя элементами
списка — результат одинаков.
...
При `linker_script: auto` фреймворк ищет шаблон `.ld.in` по четырём вариантам имени — от точного к общему:

| Приоритет | Пример для STM32G474RETx |
| --- | --- |
| 1 — точное совпадение | `STM32G474RE_FLASH.ld.in` |
| 2 — корпус заменён на X (стиль CubeMX для H7) | `STM32G474XE_FLASH.ld.in` |
...
### Папка поиска скрипта (`linker_script_dir`)

По умолчанию и шаблон, и явно указанный скрипт ищутся в корне проекта. Параметр
`linker_script_dir` добавляет папку, которая проверяется первой. Это удобно, когда
скрипты для разных MCU лежат в отдельных каталогах:

```yaml
profiles:
  F411:
    linker_script_dir: "F411"   # сначала F411/, затем корень проекта
  G474:
    linker_script_dir: "G474"
```

Параметр можно задать и на базовом уровне — тогда он действует для всех профилей,
а профиль при необходимости его переопределит.

### Пользовательский скрипт

```yaml
linker_script: "STM32H723VG_FLASH.ld"  # ищется в linker_script_dir, затем в корне
```
...
Обе семантики можно комбинировать в одном профиле: сначала применяется замена,
