...
**Рекомендация:** Располагайте эту секцию в самом **конце области FLASH**. Это удобно как для самого разработчика, так и для утилит обновления, загрузчиков и скриптов верификации, так как контрольную сумму всегда известно где искать, а вычисление CRC для полного образа прошивки возвратит ноль по свойствам алгоритма.
...
Фреймворк только *записывает* CRC в файл. Проверку при запуске вы должны реализовать самостоятельно. Ниже приведен пример функции проверки, которую следует вызывать в начале `main()` после инициализации тактирования и UART:
