...
| 2. Build modes | Bare metal without CMSIS/HAL and with project-owned startup; CMSIS without HAL; explicit/template linker scripts; profiles on one MCU | Implemented in `codex/firmware-build-modes`: four extra profiles, own bare startup and shared runtime assertions. Total: 42 builds and 48 checks per simulator. No new clock/peripheral configuration. |
