...
**Change in 0.9.3** (`3c6bfa3`; spec 4.13.4): the reference is the MCU RAM, CCRAM and RAM_SHARE total for the core, so F3/G4 CCM memory no longer shows a false excess. For the script generated by stm32-cmake the log says it was not checked instead of reporting an unchecked match; without recognised RAM regions only a warning is printed.
...
**Checks (partial coverage):** `configure.linker-template`, `configure.diagnostics-toggle`, `configure.diagnostics-unrecognized-ram`, `configure.h7-single-core-default`, `configure.h7-dual-core`, `configure.h5-cmsis-hal-freertos-external`. [Test manifest](../../../../tests/cases.json).
