...
It is separate from the 139 configure scenarios: **180 builds, 168 QEMU runs and 192 Renode runs**:
thirteen profiles per target (F103, F030), one corrupted copy per target and four H7/H5 profiles (Renode only) per tool pair; three xPack GCC versions × two CMake versions from the
[lockfile](../../ci/dependencies.lock.json), CubeF1 1.8.7, CubeF0 1.11.6, QEMU 11.0.0.
The Windows QEMU 11.1.0 installation was also checked locally; CI uses the pinned image.

## Targets

Each target builds every profile of the same fixture (spec 8.8.8). F103 case names
are unchanged; other targets have a prefix: `f030:success`. Families are added one
at a time, each with a full local check.

| Target | MCU | FLASH/RAM | Core | QEMU | Renode | FreeRTOS from Cube |
| --- | --- | --- | --- | --- | --- | --- |
| f103 | STM32F103C8T6 | 64/20 KiB | Cortex-M3 | netduino2 | f103-smoke | V10.3.1 |
| f030 | STM32F030R8T6 | 64/8 KiB | Cortex-M0 | netduino2 (M3 core) | f030-smoke (cortex-m0) | V10.0.1 |

In QEMU, F030 runs on the netduino2 Cortex-M3: Thumb-1 code is compatible and the
memory map fits. Only Renode checks the M0 core. Linker scripts and minimal
`hal_conf.h` files for targets other than F103 are generated by
`tests/firmware/semihosting/targets/make_targets.py`.

## Contract

Builds must produce nonempty ELF/BIN/HEX/MAP/LSS. ELF inspection verifies ELF32 ARM,
Thumb reset/entry agreement, an aligned initial stack pointer in target RAM, and
load segments within the target FLASH/RAM limits (F103C8: 64/20 KiB), including .data LMA.
...
For each tool pair and target, a copy of success ELF has one bit changed in .fw_version.
...
156 run builds, 168 runs per simulator. Reports distinguish thirteen profiles from fourteen executions per target.
...
[f030-smoke.repl](../../tests/firmware/renode/f030-smoke.repl) is the same model with
Cortex-M0 and 8 KiB RAM.

The ARM_CM0 port of FreeRTOS V10.0.1 (CubeF0) sets SysTick up in its own function, without
the weak `vPortSetupTimerInterrupt`, and clears VAL before writing LOAD. Renode then reloads
the counter from the previous LOAD (0xFFFFFF), and the first tick comes after about 2 s.
`freertos_tasks.c` therefore writes LOAD before starting the scheduler.
