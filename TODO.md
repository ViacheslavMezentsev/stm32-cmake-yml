...
- [ ] Текущая ветка `claude/docker-image-cache`: кэш слоёв BuildKit (`type=gha`) для Configure и Firmware; Environment/Emulation собирают с нуля и обновляют кэш. Ожидает CI.
...
- [ ] Current branch `claude/docker-image-cache`: BuildKit layer cache (`type=gha`) for Configure and Firmware; Environment/Emulation build from scratch and refresh it. Awaiting CI.
